...
class DiscountCalculator:
    ...
    def apply_weekday_weekend_discount(self):
        """평일 및 주말 할인 적용."""
        if self.date % 7 in [0, 1, 2, 3, 4]:  # 평일 (일~목)
            discount = sum(
                self.order.get(d, 0) * 2023 for d in ["초코케이크", "아이스크림"]
            )
            if discount:
        ...
        # 주말 할인 (금, 토)
        discount_list = ["티본스테이크", "바비큐립", "해산물파스타", "크리스마스파스타"]
        discount = sum(self.order.get(m, 0) * 2023 for m in discount_list)
        if discount:
